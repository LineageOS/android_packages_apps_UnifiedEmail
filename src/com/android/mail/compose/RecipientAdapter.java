...
implements OnSharedPreferenceChangeListener {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        if (!cachedAddresses.contains(address)) {
                            SuggestionEntry entry = new SuggestionEntry(
                                    suggestionId, displayName, address, name);
                            entries.add(entry);
    ...
    protected void onAddSuggestion(RecipientEntry entry) {
        super.onAddSuggestion(entry);
        new AsyncTask<RecipientEntry, Void, Boolean>() {
            @Override
            protected Boolean doInBackground(RecipientEntry... entries) {
                return createSuggestedContact(entries[0]);
            }
            @Override
    ...
    protected void onDeleteSuggestion(final RecipientEntry entry) {
        super.onDeleteSuggestion(entry);
        new AsyncTask<RecipientEntry, Void, Void>() {
            @Override
            protected Void doInBackground(RecipientEntry... entries) {
                deleteSuggestedContact(entry);
                return null;
            }
            @Override
    ...
    private boolean createSuggestedContact(RecipientEntry entry) {
        ...
        ops.add(ContentProviderOperation.newInsert(RawContacts.CONTENT_URI)
                ...
                .withValue(RawContacts.ACCOUNT_NAME, null/*mAccount.getAccountId()*/)
                .build());
    ...
    private void deleteSuggestedContact(RecipientEntry entry) {
        ContentResolver cr = getContext().getContentResolver();
