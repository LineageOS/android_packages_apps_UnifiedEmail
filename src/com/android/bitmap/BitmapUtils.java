...
public abstract class BitmapUtils {
     ...
     * @param dstSliceH     the height extent (in destination coordinates) to exclude when
     *                      cropping. You would typically pass dstH,
     *                      unless you are trying to normalize different items to the same
     *                      vertical crop range.
     * @param sampleSize    a scaling factor that rect calculation will only use if it's more
     *                      aggressive than regular scaling
     * @param vertSliceFrac vertical slice fraction determines the vertical center point for the
     *                      crop rect. Range is from [0.0, 1.0]. To perform a vertically centered
     *                      crop, use 0.5. Otherwise, see absoluteFrac.
     * @param absoluteFrac  determines how the vertSliceFrac affects the vertical center point.
     *                      If this parameter is true, the vertical center of the resulting
     *                      output rectangle will be exactly [vertSliceFrac * srcH],
     *                      with care taken to keep the bounds within the source rectangle. If
     *                      this parameter is false, the vertical center will be calculated so
     *                      that the values of vertSliceFrac from 0.0 to 1.0 will linearly cover
     *                      the entirety of the source rectangle.
    ...
    public static void calculateCroppedSrcRect(final int srcW, final int srcH, final int dstW,
            ...
            final boolean absoluteFrac, final Rect outRect) {
        if (sampleSize < 1) {
            sampleSize = 1;
        }
